...
namespace ORB_SLAM2 {
    ...
    MapPoint::MapPoint(const cv::Mat &Pos, Map *pMap, Frame *pFrame, const int &idxF) :
            ...
            mnFound(1), mbBad(false), mpReplaced(NULL), mpMap(pMap), mbToBeSerialized(true) {
        Pos.copyTo(mWorldPos);
...
//            debug("mp {} AddObservation kf {} idx {}", this->mnId, pKF->mnId, idx);
            if (idx >= pKF->mDescriptors.rows) {
                warn("observation index {} > mDescriptors.rows {}", idx, pKF->mDescriptors.rows);
            }
        }

        if (!pKF || mObservations.find(pKF) != mObservations.end())
            return;
...
//            mObservations.erase(pKF);
    ...
    void MapPoint::SetupSerializationVariable() {
        ...
        for (auto &itr: mObservations) {
            if (itr.first) {
                ...
                warn("Storing MapPoint {}: observations empty", itr.second);
            }
        }
    ...
    void MapPoint::RestoreSerialization() {
        ...
        for (auto &itr: mIdObservations) {
            ...
            if (!pKF) {
                if (itr.first != (unsigned long)(-1)) {
                    mpMap->restorationMPQueue.insert(this->mnId);
                    warn("Restoring MapPoint {}: observation {} empty", this->mnId, itr.first);
                }
            } else {
    ...
    unsigned long MapPoint::GetOriginMapId() {
