...
//    long unsigned int KeyFrame::nNextId = 0;
    ...
    KeyFrame::KeyFrame() :
            ...
            mbBad(false), mHalfBaseline(0.0), mpMap(NULL), mbToBeSerialized(true) {
...
//        debug("AddMapPoint MP {} idx {}", pMP->mnId, idx);
    ...
    void KeyFrame::EraseMapPointMatch(MapPoint *pMP, bool bAddUpdate) {
        ...
        auto idx = pMP->GetIndexInKeyFrame(this);
        if (idx < mvpMapPoints.size()) {
            if (bAddUpdate) {
                mpMap->AddUpdate(new KeyFrameUpdate<size_t>(this->mnId, __func__, size_t(idx)));
            }
    ...
    void KeyFrame::ReplaceMapPointMatch(const size_t &idx, MapPoint *pMP, bool bAddUpdate) {
        ...
        auto id = pMP ? pMP->mnId : (unsigned long) (-1);
        if (bAddUpdate) {
...
//            if (mnId != 0) {
    ...
    void KeyFrame::SetBadFlag(bool bAddUpdate) {
        {
            ...
            if (bAddUpdate) {
                mpMap->AddUpdate(new KeyFrameUpdate<int>(
                        this->mnId,
                        __func__,
                        0));
            }
    ...
    float KeyFrame::GetMinCovisibilityScore() {
        ...
        for (size_t i = 0; i < vpConnectedKeyFrames.size(); i++) {
            ...
            float score = mpKeyFrameDB->mpVoc->score(CurrentBowVec, BowVec);
    ...
    void KeyFrame::SetupSerializationVariable() {
        {
            ...
            for (auto &mp: mvpMapPoints) {
                mvnMapPointIds.push_back(mp ? mp->mnId : (unsigned long)(-1));
            }
        }

        {
            ...
            for (auto &kf: mvpOrderedConnectedKeyFrames) {
                mvnOrderedConnectedKeyFrameIds.push_back(kf ? kf->mnId : (unsigned long)(-1));
            }
    ...
    void KeyFrame::RestoreSerialization() {
        ...
        { // lock feature
            ...
            for (auto &id: mvnMapPointIds) {
                ...
                if (!mp && id != (unsigned long)(-1)) {
                    mpMap->restorationKFQueue.insert(this->mnId);
                    warn("keyframe {} mappoint {} null", mnId, id);
                }
            }
        } // end lock feature
        ...
        { // lock connection
            ...
            for (auto &id: msnLoopEdgeIds) {
                ...
                if (!kf && id != (unsigned long)(-1)) {
                    mpMap->restorationKFQueue.insert(this->mnId);
                    warn("keyframe {} loop edge {} null", mnId, id);
                }
            }
        ...
        if (mnId != 0 && !mpParent && mnParentId != (unsigned long)(-1)) {
            warn("keyframe {} parent {} null", mnId, mnParentId);
        }
    }
