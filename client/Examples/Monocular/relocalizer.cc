...
void loadMap(const string &filename, Map *&pMap, KeyFrameDatabase *&pKeyFrameDatabase, ORBVocabulary *&pVoc) {
    ...
    for (auto it: allKFs) {
        ...
        pMap->RegisterKeyFrame(itr);
        itr->mpKeyFrameDB = pKeyFrameDatabase;
        itr->ComputeBoW();
//        pkfDB->add(itr);
        itr->mpMap = pMap;
//        pKeyFrameDatabase->add(itr);
    }
...
//    auto vpMPs = pMap->GetAllMapPoints();
    ...
    cout << "Keyframes: "<< pMap->KeyFramesInMap() << endl;
    cout << "MapPoint: "<< pMap->MapPointsInMap() << endl;
}
...
void LoadImagesTUM(const string &strImagePath, const string &strPathTimes, vector<string> &vstrImageFilenames, vector<double> &vTimestamps)
{
    ifstream f;
...
int main(int argc, char **argv) {
    if (argc != 7) {
        cerr << endl << "Usage: ./relocalizer map_file_path setting_file_path voc_path img_sequence path_to_times_file dataset" << endl;
        return 1;
    }
    ...
    for (size_t i = 0; i < imageCount; ++i) {
        im = cv::imread(filenames[i], CV_LOAD_IMAGE_UNCHANGED);
        ...
        for (const auto &point: mCurrentFrame.mvKeys) {
            cv::Point2f pt1,pt2;
            pt1.x=point.pt.x-r;
            pt1.y=point.pt.y-r;
            pt2.x=point.pt.x+r;
            ...
            cv::circle(im,point.pt,2,cv::Scalar(0,255,0),-1);
        }
//        cv::imshow("ORB-SLAM2: Current Frame", im);
    ...
    int matchedCount = count_if(results.begin(), results.end(), [](bool result){
        return result;
    });
    ...
    cout << "matched / total = " << matchedCount << " / " << imageCount << endl;
